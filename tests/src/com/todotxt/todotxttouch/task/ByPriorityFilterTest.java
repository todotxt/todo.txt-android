/**
 *
 * Todo.txt Touch/src/com/todotxt/todotxttouch/task/ByPriorityFilterTest.java
 *
...
public class ByPriorityFilterTest extends TestCase {
    ...
    public void testConstructor_valid() {
        ByPriorityFilter filter = new ByPriorityFilter(Arrays.asList(Priority.A, Priority.B));
        assertNotNull(filter.getPriorities());
    ...
    public void testFilter_oneFilterPriority_noTaskPriorities() {
        ByPriorityFilter filter = new ByPriorityFilter(Arrays.asList(Priority.A));
    ...
    public void testFilter_noFilterPriority_oneTaskPriorities() {
        ByPriorityFilter filter = new ByPriorityFilter(null);
        assertTrue("apply was not true", filter.apply(new Task(1, "(A) hello world")));
    }
    ...
    public void testFilter_oneFilterPriority_differentTaskPriority() {
        ByPriorityFilter filter = new ByPriorityFilter(Arrays.asList(Priority.A));
        assertFalse("apply was not false", filter.apply(new Task(1, "(B) hello world")));
    }
    ...
    public void testFilter_multipleFilterPriority_oneDifferentTaskPriority() {
        ByPriorityFilter filter = new ByPriorityFilter(Arrays.asList(Priority.A, Priority.B));
        assertFalse("apply was not false", filter.apply(new Task(1, "(C) hello world")));
    }
}
