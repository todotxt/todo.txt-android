...
public class DropboxSyncClient implements RemoteClient, RemoteTaskRepository {
	...
	public void init(File withLocalFile) {
		try {
			api.putFile(Constants.DROPBOX_MODUS, preferences.getString(
					"todotxtpath",
					app.getResources().getString(
							R.string.TODOTXTPATH_defaultPath)), withLocalFile);
		} catch (Exception e) {
	...
	public ArrayList<Task> load() {
		try {
			DropboxAPI.FileDownload file = api.getFileStream(
					Constants.DROPBOX_MODUS,
					preferences.getString("todotxtpath", app.getResources()
