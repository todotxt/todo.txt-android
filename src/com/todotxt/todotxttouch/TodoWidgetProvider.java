...
public class TodoWidgetProvider extends AppWidgetProvider {
	...
	private final int[][] id = {
			...
			{ R.id.todoWidget_IdTask4, R.id.todoWidget_PrioTask4,
					R.id.todoWidget_TextTask4 } };
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (Constants.INTENT_WIDGET_UPDATE.equals(intent.getAction())) {
			...
			updateWidgetContent(context, AppWidgetManager.getInstance(context),
					null, null);
		}
	}
	...
	private void updateWidgetContent(Context context,
		...
		for (int i = 0; i < TASKS_TO_DISPLAY; i++) {
			...
			// text
			// TODO: use styles from widget theme, like in main task list
			// display
			...
			// remoteViews.setTextColor(id[i][TASK_TEXT],
			...
			remoteViews.setTextViewText(id[i][TASK_PRIO], task.getPriority()
					.inListFormat());
