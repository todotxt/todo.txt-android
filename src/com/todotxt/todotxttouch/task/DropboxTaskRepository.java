...
class DropboxTaskRepository implements RemoteTaskRepository {
	...
	public DropboxTaskRepository(TaskBagImpl.Preferences preferences,
	...
	public void init(File withLocalFile) {
		try {
			dropboxApi.putFile(Constants.DROPBOX_MODUS,
					preferences.todoFileDirectory, withLocalFile);
		} catch (Exception e) {
	...
	public ArrayList<Task> load() {
		try {
			DropboxAPI.FileDownload file = dropboxApi.getFileStream(
					Constants.DROPBOX_MODUS, preferences.todoFileDirectory
							+ "/" + TODO_TXT_REMOTE_FILE_NAME, null);
			if (file.isError()) {
	...
	public void store(ArrayList<Task> tasks) {
		TaskIo.writeToFile(tasks, TODO_TXT_TMP_FILE,
				preferences.useWindowsLineBreaks);
		dropboxApi.putFile(Constants.DROPBOX_MODUS,
				preferences.todoFileDirectory, TODO_TXT_TMP_FILE);
	}
}
